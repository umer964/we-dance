...
public class SlowFoxCounterHelper {
    ...
    public init(musicBeat: [[Double]], klausData: [[Double]],rate: Double) {
        ...
        musicHelperQuick?.initPlayerWithAudio(file: "SF_2", rate: self.rate)
        musicHelperQuick2?.initPlayerWithAudio(file: "SF_2", rate: self.rate)
        musicHelperQuickAnd?.initPlayerWithAudio(file: "SF_2and", rate: self.rate)
        musicHelperSlow?.initPlayerWithAudio(file: "SF_1", rate: self.rate)
    }
    ...
    private func setupTimerWithCurrentTime(time: TimeInterval, speed: Double){
        ...
        for beat in self.klausData {
            ...
            if(beatNumber == 1.0 && j == 0){
                ...
                //0.066
                timeForNext = timeForNext + ((1.0) / speed)
            ...
            if(beatNumber == 2.0 && j == 0){
                ...
                time = timeDilation + (0.0)
                //0.066
                timeForNext = timeForNext + (0.5 / speed)
