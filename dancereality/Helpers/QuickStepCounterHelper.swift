//
//  QuickStepCounterHelper.swift
//  dancereality
//
//  Created by Saad Bin Khalid on 29.02.24.
//
...
public class QuickStepCounterHelper {
    ...
    public init(musicBeat: [[Double]], klausData: [[Double]],rate: Double) {
        ...
        musicHelperQuick?.initPlayerWithAudio(file: "QS_2", rate: self.rate)
        musicHelperQuick2?.initPlayerWithAudio(file: "QS_2", rate: self.rate)
        musicHelperQuickAnd?.initPlayerWithAudio(file: "SF_2and", rate: self.rate)
        musicHelperSlow?.initPlayerWithAudio(file: "QS_1", rate: self.rate)
    }
    ...
    private func setupTimerWithCurrentTime(time: TimeInterval, speed: Double){
        ...
        for beat in self.klausData {
            ...
            if(beatNumber == 1.0 && j == 0){
                ...
                //0.1
                timeForNext = timeForNext + ((0.6) / speed)
            ...
            if(beatNumber == 2.0 && j == 0){
                ...
                time = timeDilation + (0.0)
                //0.1
                timeForNext = timeForNext + ((0.3) / speed)
    ...
    @objc func timerCall(sender: Timer){
        ...
        print("BeatPlayed:" + String(number))
